...
# NOTE: Data loading is done with int16 instead of uint16 I did in training.

# ==========================================
# VERSION CONTROL & CONFIG DICTIONARY
# ==========================================
WANDB_PROJECT = "behzadGPT"
WANDB_RUN_NAME = "1_base_fine_tuning"
CONFIG = {
    # Architecture
    "vocab_size": 8192,
    "d_model": 448,
    ...
    # Data & System
    "load_train_data_x_path": "/content/drive/MyDrive/behzadGPT/dataset/train/x_train_fine_tune.bin",
    "load_train_data_y_path": "/content/drive/MyDrive/behzadGPT/dataset/train/y_train_fine_tune.bin",
    "load_val_data_x_path": "/content/drive/MyDrive/behzadGPT/dataset/validation/x_valid_fine_tune.bin",
    "load_val_data_y_path": "/content/drive/MyDrive/behzadGPT/dataset/validation/y_valid_fine_tune.bin",
    "train_data_x_path": "/content/x_train_fine_tune.bin",
    "train_data_y_path": "/content/y_train_fine_tune.bin",
    "val_data_x_path": "/content/x_valid_fine_tune.bin",
    "val_data_y_path": "/content/y_valid_fine_tune.bin",
    "checkpoint_path": "/content/drive/MyDrive/behzadGPT/fine_tune_checkpoint/",
    "pretrained_model_path": "/content/model.pth",
    "model_path": "/content/drive/MyDrive/behzadGPT/model/model.pth",
    "num_workers": 4,
    "seed": 42,
    "eval_interval": 500,
...
class Data(Dataset):
    def __init__(self, path, seq_len):
        ...
        self.data = np.memmap(self.path[0], dtype=np.int16, mode="r")
        self.label = np.memmap(self.path[1], dtype=np.int16, mode="r")
    ...
    def __getitem__(self, index):
        ...
        input = self.data[start_idx : start_idx + self.seq_len].astype(np.int64)
        input = torch.from_numpy(input)
        start_idx += 1
        output = self.label[start_idx : start_idx + self.seq_len].astype(np.int64)
...
if __name__=="__main__":
    ...
    if os.path.exists(CONFIG["train_data_x_path"]) and os.path.exists(CONFIG["train_data_y_path"]) and os.path.exists(CONFIG["val_data_x_path"]) and os.path.exists(CONFIG["val_data_y_path"]) and os.path.exists(CONFIG["model_path"]):
        print("Dataset already downloaded.")
    else:
        !cp "{CONFIG['load_train_data_x_path']}" /content/
        !cp "{CONFIG['load_train_data_y_path']}" /content/
        !cp "{CONFIG['load_val_data_x_path']}" /content/
        !cp "{CONFIG['load_val_data_y_path']}" /content/
        !cp "{CONFIG['model_path']}" /content/
    ...
    train_dataset = Data([CONFIG["train_data_x_path"], CONFIG["train_data_y_path"]], CONFIG["context_length"])
    val_dataset = Data([CONFIG["val_data_x_path"], CONFIG["val_data_y_path"]], CONFIG["context_length"])
    ...
    model.to(device)
    model.load_state_dict(torch.load(CONFIG["pretrained_model_path"], map_location=device)["model_state_dict"])

    loss_func = nn.CrossEntropyLoss(label_smoothing=CONFIG["label_smoothing"], ignore_index = -100)
        ...
        if (i + 1) % CONFIG["accumulate"] == 0 or (i + 1) == len(train_loader):
            ...
            if global_counter % eval_interval == 0 or global_counter == batch_count:
                ...
                if global_counter == batch_count:
                    train_loss /= (batch_count % eval_interval) or eval_interval
                else:
